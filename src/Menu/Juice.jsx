...
import imageData from './utilss/juice.json';
  ...
  const handleSearch = (e) => {
    ...
    if (term === '') {
      ...
      const filtered = imageData.filter(item => 
        ...
        (item.rate && item.rate.toString().toLowerCase().includes(term))
      );
  ...
  return (
    <div className="w-full min-h-screen bg-orange-50 py-8">
      <div className="flex flex-col md:flex-row md:items-center md:justify-between mb-8 px-4 md:px-20">
        <div>
          <h1 className='font-bold text-4xl text-orange-800'>Juice</h1>
          <p className='font-bold text-xl mt-4 text-orange-700'>Taste these delectable classics, delectable juice to make your day.</p>
        </div>
        ...
        <div className="mt-6 md:mt-0 md:w-96">
          <div className="relative">
            ...
            <input
              ...
              placeholder="Search juice by name, place or price..."
              value={searchTerm}
      ...
      <div className="pb-10 mt-10">
        {filteredData.length === 0 ? (
          <div className="text-center py-10">
            <p className="text-xl text-orange-700">No juice found matching your search.</p>
          </div>
        ) : (
            ...
            {filteredData.map((item, index) => (
              <div key={index} className="rounded-lg overflow-hidden shadow-lg bg-white transition-transform duration-300 hover:shadow-xl">
                ...
                <div className="p-4">
                  ...
                  <div className="text-orange-700 font-semibold mt-2">{item.rate}</div>
                </div>
              </div>
            ))}
          </div>
